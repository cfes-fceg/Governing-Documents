"""
constitution_fr_to_docs.py — Write constitution-fr.json to a Google Doc.

Usage:
    python constitution_fr_to_docs.py <DOCUMENT_ID>
    # or:
    GOOGLE_DOC_ID=<id> python constitution_fr_to_docs.py
...
INPUT_FILE = HERE / "constitution-fr.json"
...
def get_document_id() -> str:
    ...
    print(
        ...
        "  Usage:  python constitution_fr_to_docs.py <DOCUMENT_ID>\n"
        "     or:  GOOGLE_DOC_ID=<id> python constitution_fr_to_docs.py"
    )
    sys.exit(1)
...
def handle_table(node: dict, index: int, requests: list) -> int:
    """
    Render a tblr/longtblr table as tab-separated lines of normal text.
    The first row is treated as a header with bold styling.
    """
    rows = node.get("rows", [])
    for row_idx, cells in enumerate(rows):
        ...
        if row_idx == 0:
            ...
            requests.append(make_bold(index, index + len(line) - 1))
        else:
            # Bold the first cell (the term) for definition tables
            if cells and cells[0]:
                requests.append(make_bold(index, index + len(cells[0])))
        index += len(line)
    return index
