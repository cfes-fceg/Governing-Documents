"""
tex_to_json_constitution_fr.py — Parse the French LaTeX constitution into JSON.

Reads documents/constitution-fr/main.tex, resolves all \\input{} includes, and
produces constitution-fr.json in the same directory as this script.
...
Handles:
  ...
  - \\item
  - \\begin{tblr} / \\end{tblr} and \\begin{longtblr} / \\end{longtblr}
...
begin_tblr_re   = re.compile(r"\\begin\{(?:long)?tblr\}.*")
end_tblr_re     = re.compile(r"\\end\{(?:long)?tblr\}")
...
# ---------------------------------------------------------------------------
# tblr / longtblr parser — produces a "table" node
# ---------------------------------------------------------------------------

def parse_tblr_rows(lines: list[str], start: int) -> tuple[list[list[str]], int]:
    """
    Consume lines from start until \\end{tblr} or \\end{longtblr}.
...
MAIN_TEX = HERE / "../../documents/constitution-fr/main.tex"
OUTPUT_FILE = HERE / "constitution-fr.json"
